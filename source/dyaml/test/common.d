 ...
 */
    void run(D)(string testName, D testFunction, string[] unittestExt, string[] skipExt = [])
    {
...
private:
    ...
    ///Unittest result.
    alias Result = Tuple!(string, "name", string[], "filenames", TestStatus,
            "kind", string, "info");
 ...
 */
    ...
    in
    {
        assert(F.length == strings.length);
    }
    do
    {
 ...
 */
    Result execute(D)(const string testName, D testFunction, string[] filenames) @trusted
    {
        static if (verbose)
        {
            writeln("===========================================================================");
 ...
 */
    ...
    {
        if (results.length > 0 && !verbose && !quiet)
        {
            write("\n");
        }
        ...
        foreach (result; results)
        {
            ...
            if (result.kind == TestStatus.Success)
            {
                continue;
            }

            if (result.kind == TestStatus.Failure)
            {
                ++failures;
            }
            else if (result.kind == TestStatus.Error)
            {
                ++errors;
            }
            writeln(result.info);
