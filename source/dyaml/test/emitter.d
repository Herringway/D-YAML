...
version (unittest)
{
    ...
    bool compareEvents(T, U)(T events1, U events2)
            if (isInputRange!T && isInputRange!U
                && is(ElementType!T == Event) && is(ElementType!U == Event))
    {
        foreach (e1, e2; zip(events1, events2))
        {
            ...
            if ([EventID.SequenceStart, EventID.MappingStart, EventID.Alias,
                    EventID.Scalar].canFind(e1.id) && e1.anchor != e2.anchor)
            {
                return false;
            }
        ...
        foreach (filename; [dataFilename, canonicalFilename])
        {
            ...
            foreach (flowStyle; [CollectionStyle.Block, CollectionStyle.Flow])
            {
                foreach (style; [ScalarStyle.Literal, ScalarStyle.Folded,
                        ScalarStyle.DoubleQuoted, ScalarStyle.SingleQuoted, ScalarStyle.Plain])
                {
                    Event[] styledEvents;
                    foreach (event; events)
                    {
                        ...
                        {
                            event = scalarEvent(Mark(), Mark(), event.anchor,
                                    event.tag, event.implicit, event.value, style);
                        }
                        else if (event.id == EventID.SequenceStart)
                        {
                            event = sequenceStartEvent(Mark(), Mark(),
                                    event.anchor, event.tag, event.implicit, flowStyle);
                        }
                        else if (event.id == EventID.MappingStart)
                        {
                            event = mappingStartEvent(Mark(), Mark(),
                                    event.anchor, event.tag, event.implicit, flowStyle);
                        }
                        styledEvents ~= event;
                    }
                    ...
                    static if (verbose)
                    {
                        writeln("OUTPUT (", filename, ", ",
                                to!string(flowStyle), ", ", to!string(style), ")");
