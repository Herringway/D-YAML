...
version (unittest)
{
    ...
    Node[] constructBinary() @safe
    {
        ...
        return [Node([pair("canonical", canonical), pair("generic", generic),
    ...
    Node[] constructBool() @safe
    {
        ...
        return [Node([pair("canonical", true), pair("answer", false),
                pair("logical", true), pair("option", true), pair("constbool", a),
                pair("imutbool", b), pair("const_bool", aa), pair("imut_bool", bb),
    ...
    {
        return [Node([pair("canonical", 685230.15L), pair("exponential",
                685230.15L), pair("fixed", 685230.15L), pair("sexagesimal", 685230.15L),
                pair("negative infinity", -real.infinity), pair("not a number", real.nan)])];
    }

    Node[] constructInt() @safe
    {
        return [Node([pair("canonical", 685230L), pair("decimal", 685230L),
                pair("octal", 685230L), pair("hexadecimal", 685230L),
                pair("binary", 685230L), pair("sexagesimal", 685230L)])];
    }

    Node[] constructMap() @safe
    {
        return [Node([pair("Block style", [pair("Clark", "Evans"),
                pair("Brian", "Ingerson"), pair("Oren", "Ben-Kiki")]), pair("Flow style", [pair("Clark",
                "Evans"), pair("Brian", "Ingerson"), pair("Oren", "Ben-Kiki")])])];
    }

    Node[] constructMerge() @safe
    {
        return [Node([Node([pair("x", 1L), pair("y", 2L)]), Node([pair("x",
                0L), pair("y", 2L)]), Node([pair("r", 10L)]), Node([pair("r",
                1L)]), Node([pair("x", 1L), pair("y", 2L), pair("r", 10L),
                pair("label", "center/big")]), Node([pair("r", 10L),
                pair("label", "center/big"), pair("x", 1L), pair("y", 2L)]), Node([pair("label",
                "center/big"), pair("x", 1L), pair("y", 2L), pair("r", 10L)]), Node([pair("x",
                1L), pair("label", "center/big"), pair("r", 10L), pair("y", 2L)])])];
    }

    Node[] constructNull() @safe
    {
        return [Node(YAMLNull()), Node([pair("empty", YAMLNull()),
                pair("canonical", YAMLNull()), pair("english", YAMLNull()),
                pair(YAMLNull(), "null key")]), Node([pair("sparse", [Node(YAMLNull()),
                Node("2nd entry"), Node(YAMLNull()), Node("4th entry"), Node(YAMLNull())])])];
    }

    Node[] constructOMap() @safe
    {
        return [Node([pair("Bestiary", [pair("aardvark", "African pig-like ant eater. Ugly."), pair("anteater",
                "South-American ant eater. Two species."), pair("anaconda",
                "South-American constrictor snake. Scaly.")]), pair("Numbers",
                [pair("one", 1L), pair("two", 2L), pair("three", 3L)])])];
    }

    Node[] constructPairs() @safe
    {
        return [Node([pair("Block tasks", Node([pair("meeting", "with team."),
                pair("meeting", "with boss."), pair("break", "lunch."),
                pair("meeting", "with client.")], "tag:yaml.org,2002:pairs")), pair("Flow tasks", Node([pair("meeting",
                "with team"), pair("meeting", "with boss")], "tag:yaml.org,2002:pairs"))])];
    }

    Node[] constructSeq() @safe
    {
        return [Node([pair("Block style", [Node("Mercury"), Node("Venus"),
                Node("Earth"), Node("Mars"), Node("Jupiter"), Node("Saturn"),
                Node("Uranus"), Node("Neptune"), Node("Pluto")]), pair("Flow style", [Node("Mercury"),
                Node("Venus"), Node("Earth"), Node("Mars"), Node("Jupiter"),
                Node("Saturn"), Node("Uranus"), Node("Neptune"), Node("Pluto")])])];
    }

    Node[] constructSet() @safe
    {
        return [Node([pair("baseball players", [Node("Mark McGwire"),
                Node("Sammy Sosa"), Node("Ken Griffey")]), pair("baseball teams",
    ...
    Node[] constructTimestamp() @safe
    {
        alias DT = DateTime;
        alias ST = SysTime;
        return [Node([pair("canonical", ST(DT(2001, 12, 15, 2, 59, 43),
                1000000.dur!"hnsecs", UTC())), pair("valid iso8601", ST(DT(2001,
                12, 15, 2, 59, 43), 1000000.dur!"hnsecs", UTC())), pair("space separated",
                ST(DT(2001, 12, 15, 2, 59, 43), 1000000.dur!"hnsecs", UTC())), pair("no time zone (Z)",
                ST(DT(2001, 12, 15, 2, 59, 43), 1000000.dur!"hnsecs", UTC())),
                pair("date (00:00:00Z)", ST(DT(2002, 12, 14), UTC()))])];
    }

    Node[] constructValue() @safe
    {
        return [Node([pair("link with", [Node("library1.dll"),
                Node("library2.dll")])]), Node([pair("link with", [Node([pair("=",
                "library1.dll"), pair("version", 1.2L)]), Node([pair("=",
                "library2.dll"), pair("version", 2.3L)])])])];
    }

    Node[] duplicateMergeKey() @safe
    {
        return [Node([pair("foo", "bar"), pair("x", 1L), pair("y", 2L),
                pair("z", 3L), pair("t", 4L)])];
    }

    Node[] floatRepresenterBug() @safe
    {
        return [Node([pair(1.0L, 1L), pair(real.infinity, 10L),
                pair(-real.infinity, -10L), pair(real.nan, 100L)])];
    }
    ...
    {
        return [Node([Node(0.0L), Node(1.0L), Node(-1.0L),
                Node(real.infinity), Node(-real.infinity), Node(real.nan), Node(real.nan)])];
    }
 ...
 */
    void testConstructor(string dataFilename, string codeDummy) @safe
    {
        ...
        enforce((base in expected) !is null, new Exception("Unimplemented constructor test: " ~ base));
